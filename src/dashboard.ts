...
import { estimateTokens, mergeStats, type RoleStat, type WorkspaceStats } from './core/stats.js';
import { ROLE_ORDER } from './init/detect.js';
import type { TmuxClient } from './tmux/client.js';
import { spawnControlWatcher, type ControlWatcher, type WatcherEvent, type WatcherOptions } from './tmux/watcher.js';
...
export const EMPTY_MESSAGE = 'No sidemux workspace exists yet. Run a command first.';
export const NORMAL_HELP = 'NORMAL · j/k/Tab move · Enter open+zoom · d kill · / search · r reload · q close';
...
export function initialDashboardState(rows: DashboardRow[] = []): DashboardState {
...
export function dashboardReducer(state: DashboardState, action: DashboardAction): DashboardState {
  switch (action.type) {
    case 'move': {
      ...
      return { ...state, cursor: clamp(state.cursor + action.delta, 0, count - 1) };
    }
    case 'setRows': {
      const next = { ...state, rows: action.rows, message: action.message ?? null };
      const count = filteredRows(next).length;
      return { ...next, cursor: count === 0 ? 0 : clamp(next.cursor, 0, count - 1) };
    }
    case 'enterInsert':
    ...
    case 'deleteSelected': {
      ...
      const count = filteredRows(next).length;
      return { ...next, cursor: count === 0 ? 0 : clamp(state.cursor, 0, count - 1) };
    }
  }
}
...
export async function runDashboard(
  ...
  const watcher = (deps.spawnWatcher ?? spawnControlWatcher)(
    config.sessionName,
    (event) => {
      if (event.type === 'died') {watcherAlive = false;}
      else {dirty = true;}
    },
  ...
  const refreshPreview = async (): Promise<void> => {
    ...
    if (row.kind === 'agent' && row.paneIds && row.paneIds.length > 0) {
      preview = await stackedPreview(client, row, state.rows, output.rows || 30);
      return;
    }
    try {
  ...
  const handleKey = async (chunk: Buffer): Promise<void> => {
    ...
    if (state.mode === 'insert') {
      if (key === '\x1b') {state = dashboardReducer(state, { type: 'escape' });}
      else if (key === '\r' || key === '\n') {await openSelected();}
      else if (key === '\x7f' || key === '\b') {state = dashboardReducer(state, { type: 'backspace' });}
      else if (isPrintable(key)) {state = dashboardReducer(state, { type: 'input', text: key });}
    } else {
      if (key === 'q' || key === '\x1b') {close();}
      else if (key === 'j' || key === '\t' || key === '\x1b[B') {state = dashboardReducer(state, { type: 'move', delta: 1 });}
      else if (key === 'k' || key === '\x1b[Z' || key === '\x1b[A') {state = dashboardReducer(state, { type: 'move', delta: -1 });}
      else if (key === '/') {state = dashboardReducer(state, { type: 'enterInsert' });}
      else if (key === 'r') {await reload();}
      else if (key === 'd') {await deleteSelected();}
      else if (key === '\r' || key === '\n') {await openSelected();}
    }
    await refreshPreview();
    if (!closed) {
  ...
  input.resume();
  input.on('data', (chunk) => void handleKey(chunk as Buffer).catch((error: unknown) => {
    state = { ...state, message: errorMessage(error) };
    render(output, state, preview, config, stats);
  }));
...
export function renderDashboard(
  ...
  const statsLines = height >= 16 ? renderStats(options.stats ?? {}, width) : [];
  const bodyHeight = Math.max(3, height - 3 - density.outerBlankLines - statsLines.length);
  ...
  const listHeight = wide ? bodyHeight : Math.max(5, Math.floor(bodyHeight * 0.44));
  const stackGap = wide ? 0 : density.stackGapLines;
  const previewHeight = wide ? bodyHeight : Math.max(3, bodyHeight - listHeight - stackGap);
  const selected = selectedRow(state);
  ...
  const list = renderPanel(
    density.compactTitles ? `pane ${rows.length}/${state.rows.length}` : `panes ${rows.length}/${state.rows.length}`,
    renderList(state, rows, Math.max(1, listInner.width), Math.max(1, listInner.height), density.rowGapLines),
    listWidth,
    listHeight,
    density,
  );
  ...
  const previewTitle = density.compactTitles
    ? selected ? `out ${selected.activePaneId}` : 'out'
    : selected ? `preview ${selected.activePaneId}` : 'preview';
  const previewPanel = renderPanel(
    previewTitle,
    renderPreview(previewText, Math.max(1, previewInner.width), Math.max(1, previewInner.height), density.rowGapLines),
    previewWidth,
    previewHeight,
    density,
  );
  const lines = [renderTopBar(header, state, rows.length, width), renderDetailBar(selected, width)];
  for (let i = 0; i < density.outerBlankLines; i += 1) {lines.push('');}

  if (wide) {
    for (let i = 0; i < bodyHeight; i += 1) {
      lines.push(`${pad(list[i] ?? '', listWidth)}${density.horizontalGap}${previewPanel[i] ?? ''}`);
    }
  ...
  for (let i = 0; i < stackGap; i += 1) {lines.push('');}
...
function render(
  ...
  output.write(
    ANSI.clear +
      renderDashboard(state, preview, width, height, { density: config.dashboardDensity, stats }),
  );
}
...
function renderList(
  ...
  if (rows.length === 0) {
    const text = state.rows.length === 0 ? state.message || EMPTY_MESSAGE : `No matches for "${state.query}"`;
  ...
  const offset = Math.max(0, Math.min(state.cursor - visibleRowCount + 1, rows.length - visibleRowCount));
  const lines: string[] = [color(pad(table.header, width), THEME.title)];
  for (const [index, row] of rows.slice(offset, offset + visibleRowCount).entries()) {
    const selected = offset + index === state.cursor;
    const accent = selected ? color('▌', THEME.accent) : color(' ', THEME.border);
    const line = `${accent} ${formatTableRow(row, table, selected)}`;
    lines.push(selected ? selectedLine(line, width) : pad(line, width));
    for (let gap = 0; gap < rowGapLines && lines.length < height; gap += 1) {lines.push('');}
  }
...
function renderPreview(value: string, width: number, height: number, rowGapLines = 0): string[] {
  ...
  for (const line of wrapLines(text, width)) {
    ...
    for (let gap = 0; gap < rowGapLines; gap += 1) {lines.push('');}
  }
  while (lines.length > 0 && lines.at(-1) === '') {lines.pop();}
  return lines.slice(-height);
}

function renderTopBar(help: string, state: DashboardState, visibleRows: number, width: number): string {
  ...
  return fitLine(`${brand} ${mode} ${color(help, THEME.faint)} ${count}`, width);
}
 ...
 */
export function renderDetailBar(row: DashboardRow | null, width: number): string {
  if (!row) {return fitLine(color('no selection', THEME.faint), width);}
  const exitSuffix =
    row.lastExitCode !== null && row.lastExitCode !== undefined ? ` exit ${row.lastExitCode}` : '';
  const segments: DetailSegment[] =
    row.kind === 'pane'
      ? [
          { text: `win ${row.windowIndex}`, bg: THEME.accent, fg: 16 },
          { text: `${row.managedName || row.windowName} ${row.activePaneId}`, bg: THEME.selected },
          { text: truncatePlain(row.description ?? '—', 48), bg: THEME.panel },
          { text: truncatePlain(row.script ?? '-', 36), bg: THEME.selected },
          { text: truncatePlain(abbreviateHome(row.dir ?? '-'), 32), bg: THEME.panel },
          { text: `${statusText(row, true)}${exitSuffix}`, bg: statusColor(row), fg: 16 },
        ]
      : [
          { text: `win ${row.windowIndex}`, bg: THEME.accent, fg: 16 },
          { text: `${row.windowName} ${row.windowId}`, bg: THEME.selected },
          { text: truncatePlain(row.description ?? '—', 32), bg: THEME.panel },
          { text: `${row.agentId ?? '-'} · pid ${row.serverPid ?? '-'}`, bg: THEME.selected },
          { text: truncatePlain(abbreviateHome(row.dir ?? '-'), 32), bg: THEME.panel },
          { text: `${statusText(row, true)}${exitSuffix}`, bg: statusColor(row), fg: 16 },
        ];
...
function renderFooter(state: DashboardState, width: number): string {
  const filter = state.query ? color(state.query, THEME.text) : color('<empty>', THEME.faint);
  ...
  return fitLine(`${color('filter', THEME.accent)}: ${filter}  ${message}`, width);
}
...
function innerSize(width: number, height: number, density: DensitySpec): { width: number; height: number } {
  if (density.style === 'compact') {return { width, height: Math.max(0, height - 1) };}
  return {
...
function renderPanel(title: string, content: string[], width: number, height: number, density: DensitySpec): string[] {
  if (height <= 0) {return [];}
  if (height === 1) {return [fitLine(title, width)];}
  if (density.style === 'compact') {return [fitLine(`${title}:`, width), ...content.map((line) => fitLine(line, width))].slice(0, height);}
  if (height === 2) {return [panelTop(title, width), panelBottom(width)];}
  const innerWidth = Math.max(0, width - 2);
  const bodyHeight = height - 2;
  const lines = [panelTop(title, width)];
  for (let i = 0; i < density.panelPadding && lines.length < height - 1; i += 1) {
    lines.push(`${color('│', THEME.border)}${' '.repeat(innerWidth)}${color('│', THEME.border)}`);
  }
  for (let i = 0; i < bodyHeight; i += 1) {
    if (lines.length >= height - 1 - density.panelPadding) {break;}
    ...
    lines.push(`${color('│', THEME.border)}${pad(padded, innerWidth)}${color('│', THEME.border)}`);
  }
...
function tableSpec(width: number): TableSpec {
  ...
  return {
    ...
    nameWidth: 0,
    agentWidth: 0,
    header: [pad('#', 3), 'S', pad('SCRIPT', scriptWidth), pad('DIR', dirWidth), 'IDS'].join(' '),
  };
}

function formatTableRow(row: DashboardRow, table: TableSpec, selected: boolean): string {
  const dim = selected ? THEME.text : THEME.dim;
  const status = color(pad(statusText(row, table.full), table.statusWidth), statusColor(row));
  const script = color(pad(truncatePlain(displayScript(row), table.scriptWidth), table.scriptWidth), dim);
  const dir = color(pad(truncatePlain(displayDir(row, table.full), table.dirWidth), table.dirWidth), dim);
  const ids = color(`${row.windowId} ${row.activePaneId}`, THEME.dim);
  const index = color(pad(row.kind === 'pane' ? '' : row.windowIndex, 3), selected ? THEME.accent2 : THEME.dim);
  if (!table.full) {return `${index} ${status} ${script} ${dir} ${ids}`;}

  const desc = color(pad(truncatePlain(row.description ?? '-', table.descWidth), table.descWidth), THEME.dim);
  const name = color(pad(truncatePlain(row.managedName || row.windowName, table.nameWidth), table.nameWidth), dim);
  const agent = color(pad(truncatePlain(row.agentId || '-', table.agentWidth), table.agentWidth), THEME.dim);
  const pid = color(pad(row.serverPid?.toString() ?? '-', 5), THEME.dim);
  const exit = color(pad(row.lastExitCode?.toString() ?? '-', 4), statusColor(row));
...
function displayDir(row: DashboardRow, full: boolean): string {
  if (!row.dir) {return '-';}
  if (full) {return row.dir;}
  return row.dir.split('/').filter(Boolean).pop() ?? row.dir;
}

function statusGlyph(row: DashboardRow): string {
  if (row.busy) {return '●';}
  if (row.lastExitCode === 0) {return '✓';}
  if (row.lastExitCode !== null && row.lastExitCode !== undefined) {return '✕';}
  return '·';
}
...
function statusText(row: DashboardRow, full: boolean): string {
  ...
  if (!full) {return icon;}
  if (row.busy) {return `${icon} run`;}
  if (row.lastExitCode === 0) {return `${icon} ok`;}
  if (row.lastExitCode !== null && row.lastExitCode !== undefined) {return `${icon} fail`;}
  return `${icon} idle`;
}

function statusColor(row: DashboardRow): number {
  if (row.busy) {return THEME.accent2;}
  if (row.lastExitCode === 0) {return THEME.accent;}
  if (row.lastExitCode !== null && row.lastExitCode !== undefined) {return THEME.danger;}
  return THEME.dim;
}
 ...
 */
export function buildRows(windows: WindowInfo[], panes: PaneInfo[]): DashboardRow[] {
  const rows: DashboardRow[] = [];
  for (const window of windows) {
    const children = panes.filter(
      (candidate) => candidate.windowId === window.windowId && candidate.managed,
    );
    const fallback =
    ...
    rows.push({
      ...
      lastExitCode: failed?.lastExitCode ?? (children.some((pane) => pane.lastExitCode === 0) ? 0 : null),
    });
...
async function stackedPreview(
  ...
  const perPane = Math.max(3, Math.floor((terminalRows - 6) / paneIds.length) - 1);
  const sections: string[] = [];
  for (const paneId of paneIds) {
    const child = rows.find(
      (candidate) => candidate.kind === 'pane' && candidate.activePaneId === paneId,
    );
