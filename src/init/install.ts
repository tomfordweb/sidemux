...
import { globalConfigPath, globalConfigTemplate, loadProjectScripts } from '../config-file.js';
import {
  classifyCommandRole,
  detectCandidates,
...
async function promptYesNo(io: InitIO, question: string, defaultYes = true): Promise<boolean> {
  const rl = createInterface({ input: io.stdin, output: io.stdout });
  try {
...
async function writeArtifacts(
  ...
  await writeFile(join(sidemuxDir, 'delegate.json'), delegateJson(selected, [...nxScripts.keys()]));
  ...
  out.write(
    '\nWrote:\n' +
      ...
      (nxScripts.size > 0 ? '  .sidemux.toml                 Nx per-project scripts\n' : '') +
 ...
 */
async function walkUpPackageManager(dir: string): Promise<PackageManager | undefined> {
...
const NX_SCAN_SKIP = new Set(['node_modules', 'dist', 'coverage', '.git', '.nx', '.worktrees']);
 ...
 */
async function scanNxProjects(dir: string, depth = 0): Promise<NxProjectInfo[]> {
  if (depth > NX_SCAN_MAX_DEPTH) {return [];}
  const entries = await readdir(dir, { withFileTypes: true }).catch(() => []);
  const projects: NxProjectInfo[] = [];

  if (depth > 0 && entries.some((e) => e.isFile() && e.name === 'project.json')) {
    const projectJson = await readJson(join(dir, 'project.json'));
    if (typeof projectJson?.name === 'string') {
      const targets = Object.keys((projectJson.targets as Record<string, unknown> | undefined) ?? {});
      ...
      projects.push({ name: projectJson.name, targets: [...new Set([...targets, ...scripts])] });
    }
  }

  for (const entry of entries) {
    if (!entry.isDirectory() || NX_SCAN_SKIP.has(entry.name)) {continue;}
...
async function writeNxScripts(
  ...
  if (Object.keys(scripts).length === 0 && !text) {return;}
...
async function detectFromDir(dir: string): Promise<DirDetection> {
  ...
  const fallbackPackageManager =
    packageJson && !packageManagerFromLockfiles(rootFiles) && !packageJson.packageManager
  ...
  for (const script of loadProjectScripts(dir).values()) {
    if (known.has(script.command) || generatedNames.has(script.name)) {continue;}
    const inferred = inferCommand(script.command);
    detected.push({ ...inferred, longRunning: inferred.longRunning || script.background });
  }
  return {
    detected,
    pm: detectPackageManager(rootFiles, packageJson?.packageManager, fallbackPackageManager),
...
async function readPriorSelection(dir: string): Promise<DelegatedCommand[] | null> {
  ...
  for (const entry of config.commands as (Partial<DelegatedCommand> | null)[]) {
    ...
    selected.push({
      ...
      role: entry.role !== undefined && ROLE_ORDER.includes(entry.role) ? entry.role : inferred.role,
...
async function uninstall(dir: string, out: NodeJS.WritableStream): Promise<void> {
  ...
  if (!hadSidemuxDir && !removedHook && !removedDirectives && !removedNxScripts && !hadMcpEntry) {
  ...
  if (removedNxScripts) {removed.push('generated Nx scripts in .sidemux.toml');}
...
export async function runInit(options: InitOptions): Promise<number> {
  ...
  if (flags.mcp && flags.closeOnSuccess) {mcpEnv.SIDEMUX_CLOSE_ON_SUCCESS = '1';}

  const nxScripts = detection.hasNx
    ? nxProjectScripts(detection.pm, await scanNxProjects(flags.dir))
    : new Map<string, string>();
  await writeArtifacts(flags.dir, selected, flags.mcp, mcpEnv, io.stdout, nxScripts);
 ...
 */
async function maybeScaffoldGlobalConfig(yes: boolean, io: InitIO): Promise<void> {
  if (yes) {return;}
  // Only offer on a real terminal — scripted/piped stdin must never block here.
  if (!(io.stdin as NodeJS.ReadStream).isTTY) {return;}
