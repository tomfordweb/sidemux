...
export function hasWorkspace(rootCwd: string): boolean {
  return (
    existsSync(join(rootCwd, "pnpm-workspace.yaml")) ||
    existsSync(join(rootCwd, "nx.json"))
  );
}
...
async function readPnpmGlobs(rootCwd: string): Promise<string[]> {
  const text = await readFile(
    join(rootCwd, "pnpm-workspace.yaml"),
    "utf8",
  ).catch(() => "");
  if (!text) {
    return [];
...
async function expandGlob(rootCwd: string, glob: string): Promise<string[]> {
  ...
  if (!glob.endsWith("*")) {
    return [];
  } // deeper patterns (a/*/b) unsupported
...
async function projectName(
  dir: string,
): Promise<{ name: string | null; hasManifest: boolean }> {
...
export async function listProjects(
  ...
  if (globs.length === 0 && existsSync(join(rootCwd, "nx.json"))) {
    globs = ["apps/*", "libs/*"];
  }

  const dirs = (
    await Promise.all(globs.map((g) => expandGlob(rootCwd, g)))
  ).flat();
