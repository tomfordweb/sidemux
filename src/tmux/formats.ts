import type { ManagedPaneClass, PaneInfo, PaneState, WindowInfo } from '../types.js';
...
export function encodeOptionValue(value: string): string {
  ...
  const needsEncoding = /[\t\n\r]|#\{/.test(value) || value === ';' || value.startsWith(ENCODED_PREFIX);
...
export function decodeOptionValue(value: string): string {
  ...
  try {
    return Buffer.from(value.slice(ENCODED_PREFIX.length), 'base64url').toString('utf8');
  } catch {
    return value;
  }
}
...
export function parsePaneList(output: string): PaneInfo[] {
  return output
    ...
    .map((line) => {
      ...
      const field = (index: number): string => parts[index] ?? '';
      const [width, height] = field(8).split('x').map((n) => Number.parseInt(n, 10));
      const title = field(5);
      const windowId = parts[9] ?? '';
      const managedOption = parts[10] ?? '';
      const managed = managedOption === '1' || title.startsWith(MANAGED_TITLE_PREFIX);
      return {
        paneId: field(0),
        target: field(1),
        sessionName: field(2),
...
export function parseWindowList(output: string): WindowInfo[] {
  return output
    ...
    .map((line) => {
      const parts = line.split(SEP);
      if (parts.length < 5) {
        throw new Error(`unexpected list-windows line: ${JSON.stringify(line)}`);
      }
      const field = (index: number): string => parts[index] ?? '';
      return {
        sessionName: field(0),
