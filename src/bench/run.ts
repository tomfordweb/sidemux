...
async function benchCommand(
  ...
  const runTimeoutMs = envDurationMs(
    "SIDEMUX_BENCH_TIMEOUT_MS",
    DEFAULT_RUN_TIMEOUT_MS,
  );
  const requestTimeoutMs = envDurationMs(
...
export function parseCommands(argv: string[]): {
  commands: string[];
  help: boolean;
} {
...
export async function runBenchmark(options: BenchOptions): Promise<number> {
  ...
  try {
    ...
    out.write(
      "sidemux benchmark: tmux not found on PATH — install tmux first.\n",
    );
    return 1;
  }
  ...
  try {
    ...
    for (const command of commands) {
      rows.push([
        `\`${command}\``,
        await benchCommand(client, command, options.cwd),
      ]);
    }
