...
describe("sidemux init", () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        scripts: { test: "vitest", build: "tsup", dev: "vite" },
      }),
    );
  ...
  test("--yes writes guard, hook, delegate.json, and directives", async () => {
    ...
    expect(
      delegate.commands.map((c: { command: string }) => c.command),
    ).toEqual(["pnpm test", "pnpm build", "pnpm dev"]);
  ...
  test("--commands overrides detection and infers unknown roles", async () => {
    ...
    await runInit({
      cwd: dir,
      argv: ["--commands", "cargo test,webpack serve"],
      io,
    });
    const delegate = JSON.parse(
      await readFile(join(dir, ".sidemux", "delegate.json"), "utf8"),
    ...
    const byCommand = Object.fromEntries(
      delegate.commands.map((c: { command: string; longRunning: boolean }) => [
        c.command,
        c.longRunning,
      ]),
    );
  ...
  test("--uninstall removes the guard, hook, and directives", async () => {
    ...
    await expect(
      stat(join(dir, ".sidemux", "delegate-guard.mjs")),
    ).rejects.toThrow();
    const settings = JSON.parse(
      await readFile(join(dir, ".claude", "settings.json"), "utf8"),
    );
    expect(JSON.stringify(settings)).not.toContain("delegate-guard.mjs");
    expect(await readFile(join(dir, "CLAUDE.md"), "utf8")).not.toContain(
      "sidemux-delegate",
    );
  });
  ...
  test("--sync --yes refreshes artifacts, keeps the recorded selection, lists new candidates", async () => {
    ...
    await runInit({
      cwd: dir,
      argv: ["--commands", "pnpm test", "--mcp", "--close-on-success"],
      io,
    });
    ...
    expect(
      await readFile(join(dir, ".sidemux", "delegate-guard.mjs"), "utf8"),
    ).toContain("matchesDelegated");
  ...
  test("interactive --sync asks about commands detected since the last init", async () => {
    ...
    const commands = delegate.commands.map(
  ...
  test("--yes with nothing detected installs the generic-only directive", async () => {
    await writeFile(
      join(dir, "package.json"),
      JSON.stringify({ scripts: { deploy: "x" } }),
    );
    const { io, text } = fakeIo();
    const code = await runInit({ cwd: dir, argv: ["--yes"], io });
    expect(code).toBe(0);
