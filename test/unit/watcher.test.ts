...
const { classifyControlLine, spawnControlWatcher } = await import('../../src/tmux/watcher.js');
...
function start(options: { socketName?: string | null } = {}): {
  ...
  const watcher = spawnControlWatcher('smux', (event) => events.push(event), options);
...
describe('spawnControlWatcher', () => {
  ...
  test('emits events per complete line, buffering partial chunks', () => {
    ...
    expect(events).toEqual([{ type: 'output', paneId: '%5' }, { type: 'topology' }]);
  });
