...
import { hasWorkspace, listProjects, resolveProject } from '../../src/core/workspace.js';
...
async function pkg(root: string, rel: string, name?: string): Promise<void> {
  ...
  await writeFile(join(dir, 'package.json'), JSON.stringify(name ? { name } : {}));
}
...
describe('workspace project resolution', () => {
  test('pnpm-workspace globs map package names and dir-basename aliases', async () => {
    ...
    expect(await resolveProject(root, 'keebgod')).toBe(join(root, 'apps/keebgod'));
    expect(await resolveProject(root, 'domain')).toBe(join(root, 'libs/domain'));
  ...
  test('nested globs (libs/maps/*) expand', async () => {
    ...
    expect(await resolveProject(root, 'tiles')).toBe(join(root, 'libs/maps/tiles'));
  });
