...
describe("globalConfigPath", () => {
  test("honors XDG_CONFIG_HOME", () => {
    expect(globalConfigPath({ XDG_CONFIG_HOME: "/xdg" })).toBe(
      "/xdg/sidemux/config.toml",
    );
  });
...
describe("loadGlobalFileConfig", () => {
  ...
  test("unknown keys warn but known keys still apply", async () => {
    ...
    expect(error).toHaveBeenCalledWith(
      expect.stringContaining('unknown key "typo_key"'),
    );
  });
...
describe("loadProjectScripts", () => {
  test("reads string and table script forms", async () => {
    ...
    expect(scripts.get("lint")).toEqual({
      name: "lint",
      command: "nx run *:lint",
      background: false,
    });
    expect(scripts.get("big")?.command).toBe("./bin/someCustomScript");
    expect(scripts.get("dev")).toEqual({
      name: "dev",
      command: "pnpm dev",
      background: true,
    });
  ...
  test("globs in script bodies pass through untouched", async () => {
    ...
    expect(loadProjectScripts(dir).get("lint")?.command).toBe(
      "nx run *:lint --fix **/*.ts",
    );
  });
