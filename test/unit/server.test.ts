...
describe('buildServer', () => {
  ...
  test('read defaults to incremental last-read', async () => {
    ...
    expect(service.read).toHaveBeenCalledWith(
      expect.objectContaining({ since: 'last-read', lines: 100, context: 2, max_bytes: 8192 }),
    );
  });
  ...
  test('send_keys, list_panes, kill round-trip', async () => {
    ...
    expect((listed.structuredContent as { panes: unknown[] }).panes).toHaveLength(1);

    const killed = await client.callTool({ name: 'kill', arguments: { pane: '%7' } });
    expect(service.kill).toHaveBeenCalledWith(expect.objectContaining({ mode: 'interrupt' }));
  ...
  test('progress token wires a reporter through to the service', async () => {
    ...
    const onProgress = (service.wait as ReturnType<typeof vi.fn>).mock.calls[0]![1] as
      | ((ms: number) => void)
      | undefined;
