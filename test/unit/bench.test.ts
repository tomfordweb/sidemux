...
const out = (fake: FakeOut): NodeJS.WritableStream => fake as unknown as NodeJS.WritableStream;

describe('parseCommands', () => {
  test('collects repeated --command values in order', () => {
    expect(parseCommands(['--command', 'pnpm test', '--command', 'pnpm lint'])).toEqual({
  ...
  test('mixed forms and interleaved flags all land in commands', () => {
    expect(parseCommands(['--command=a', '--command', 'b', '--command=c'])).toEqual({
      commands: ['a', 'b', 'c'],
      help: false,
    });
  });
  ...
  test('--help and -h set the help flag', () => {
    ...
    expect(parseCommands(['-h', '--command', 'x'])).toEqual({ commands: ['x'], help: true });
  });
