...
describe("dashboard state", () => {
  ...
  test("backspace edits filter query", () => {
    ...
    expect(filteredRows(state).map((row) => row.windowName)).toEqual([
      "lint",
      "test",
    ]);
  });
  ...
  test("narrow rendering stacks windows above preview", () => {
    ...
    const plain = stripAnsi(output);
    expect(plain.indexOf("panes 3/3")).toBeLessThan(
      plain.indexOf("preview %10"),
    );
  ...
  test("compact density shows more rows than spacious at the same size", () => {
    ...
    const compact = stripAnsi(
      renderDashboard(state, "", 120, 12, { density: "compact" }),
    );
    const spacious = stripAnsi(
      renderDashboard(state, "", 120, 12, { density: "spacious" }),
    );

    expect(countRenderedJobs(compact)).toBeGreaterThan(
      countRenderedJobs(spacious),
    );
  ...
  test("spacious density adds blank padding while keeping selected row semantics", () => {
    let state = initialDashboardState(rows);
    state = dashboardReducer(state, { type: "move", delta: 1 });
    const spacious = renderDashboard(state, "", 120, 14, {
      density: "spacious",
    });
    const normal = renderDashboard(state, "", 120, 14, { density: "normal" });

    expect(stripAnsi(spacious)).toContain("▌ 1");
    expect(blankLineCount(stripAnsi(spacious))).toBeGreaterThan(
      blankLineCount(stripAnsi(normal)),
    );
  });
});
...
describe("runDashboard key loop", () => {
  ...
  test("failed delete surfaces the tmux error in the footer", async () => {
    ...
    await press(stdin, "d");
    expect(stripAnsi(stdout.chunks.at(-1) ?? "")).toContain(
      "can't find window: @1",
    );
...
describe("buildRows", () => {
  ...
  test("pane rows render indented under their agent row", () => {
    ...
    const plain = stripAnsi(
      renderDashboard(initialDashboardState(built), "", 120, 14),
    );
    expect(plain).toContain("└ pnpm lint");
...
describe("detail bar and descriptions", () => {
  test("pane rows carry the run description into rows and the DESC column", () => {
    ...
    const plain = stripAnsi(
      renderDashboard(initialDashboardState(built), "", 320, 14),
    );
