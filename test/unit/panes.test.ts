...
function stubClient(overrides: Partial<TmuxClient> = {}): TmuxClient {
  ...
  >();
  const createPane = async (cwd: string, windowName?: string, windowId?: string) => {
  ...
  return {
    resolveTarget: vi.fn(async (t: string) => {
      if (panes.has(t)) {return t;}
      for (const pane of panes.values()) {
        if (pane.target === t) {return pane.paneId;}
      }
      return t;
    }),
    paneExists: vi.fn(async (t: string) => panes.has(t)),
    panePath: vi.fn(async (paneId: string) => panes.get(paneId)?.cwd ?? '/somewhere'),
    splitWindow: vi.fn(async (cwd: string) => createPane(cwd)),
    splitWindowInWindow: vi.fn(async (_session: string, windowIndex: string, cwd: string) => {
      const host = [...windows.values()].find((w) => w.windowIndex === windowIndex);
      return createPane(cwd, undefined, host?.windowId);
    }),
    newSession: vi.fn(async (_session: string, cwd: string, _shell?: string, windowName?: string) =>
      createPane(cwd, windowName),
    ),
    newWindow: vi.fn(async (_session: string, cwd: string, _shell?: string, windowName?: string) =>
      createPane(cwd, windowName),
    ),
    hasSession: vi.fn(async (sessionName: string) =>
      [...windows.values()].some((window) => window.sessionName === sessionName),
    ),
    listPanes: vi.fn(async () =>
      [...panes.values()].map((pane) => ({
        ...
        managed: pane.options['@smux_managed'] === '1' || pane.title.startsWith('smux:'),
        ...
        paneClass:
          pane.options['@smux_class'] === 'persistent' || pane.options['@smux_class'] === 'oneshot'
    ...
    listWindows: vi.fn(async (_session?: string) =>
      [...windows.values()].map((window) => ({
        ...window,
        activePaneId: [...panes.values()].find((pane) => pane.windowId === window.windowId)?.paneId ?? '%0',
    ...
    setPaneOption: vi.fn(async (paneId: string, name: string, value: string) => {
      const pane = panes.get(paneId);
      if (pane) {pane.options[name] = value;}
    }),
    unsetPaneOption: vi.fn(async (paneId: string, name: string) => {
      const pane = panes.get(paneId);
      if (pane) {delete pane.options[name];}
    }),
    updatePane: vi.fn(async (paneId: string, title: string, options: OptionWrite[]) => {
      const pane = panes.get(paneId);
      if (!pane) {return;}
      pane.title = title;
      for (const option of options) {
        if (option.value === null) {delete pane.options[option.name];}
        else {pane.options[option.name] = option.value;}
      }
    }),
    paneWindow: vi.fn(async (paneId: string) => panes.get(paneId)?.windowId ?? '@1'),
    setWindowOption: vi.fn(async (windowId: string, name: string, value: string) => {
      const window = windows.get(windowId);
      if (window) {window.options[name] = value;}
    }),
    setWindowOptions: vi.fn(async (windowId: string, options: OptionWrite[]) => {
      const window = windows.get(windowId);
      if (!window) {return;}
      for (const option of options) {
        if (option.value === null) {delete window.options[option.name];}
        else {window.options[option.name] = option.value;}
      }
    }),
    unsetWindowOption: vi.fn(async (windowId: string, name: string) => {
      const window = windows.get(windowId);
      if (window) {delete window.options[name];}
    }),
    renameWindow: vi.fn(async (windowId: string, name: string) => {
      const window = windows.get(windowId);
      if (window) {window.windowName = name;}
    }),
    ...
    killWindow: vi.fn(async (windowId: string) => {
      ...
      for (const [paneId, pane] of panes) {
        if (pane.windowId === windowId) {panes.delete(paneId);}
      }
    }),
    ...overrides,
...
async function backdate(client: TmuxClient, paneId: string, ageMs: number): Promise<void> {
  await client.setPaneOption(paneId, '@smux_last_used_at', String(Date.now() - ageMs));
}
...
describe('PaneAllocator', () => {
  ...
  test('creates the configured detached workspace session', async () => {
    const client = stubClient();
    const allocator = new PaneAllocator(client, loadConfig({ SIDEMUX_AGENT_ID: 'agent-1' }), {}, '/proj');
    const acquired = await allocator.acquire({});
    expect(acquired.created).toBe(true);
    expect(client.newSession).toHaveBeenCalledWith('smux', '/proj', undefined, 'agent-1');
  });
  ...
  test('existing workspace session gets a new window per agent', async () => {
    ...
    expect(client.newWindow).toHaveBeenCalledWith('smux', '/proj', undefined, 'agent-1');
  });
  ...
  test('groups multiple pane names under the same agent window', async () => {
    ...
    expect(client.splitWindowInWindow).toHaveBeenCalledWith('smux', '0', '/proj', undefined);
  });

  test('creates a separate window for another agent', async () => {
    const client = stubClient();
    const firstAgent = new PaneAllocator(client, loadConfig({ SIDEMUX_AGENT_ID: 'agent-1' }), {}, '/proj');
    const secondAgent = new PaneAllocator(client, loadConfig({ SIDEMUX_AGENT_ID: 'agent-2' }), {}, '/proj');

    await firstAgent.acquire({ name: 'lint' });
    await secondAgent.acquire({ name: 'lint' });

    expect(client.newSession).toHaveBeenCalledTimes(1);
    expect(client.newWindow).toHaveBeenCalledWith('smux', '/proj', undefined, 'agent-2');
  });
  ...
  test('explicit cwd param beats the default', async () => {
    ...
    expect(client.newSession).toHaveBeenCalledWith('smux', '/elsewhere', undefined, expect.any(String));
  });
  ...
  test('pane header title carries the command and pane id', async () => {
    ...
    expect(client.updatePane).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([{ name: '@smux_label', value: 'test · pnpm test · %10' }]),
    );
  });
  ...
  test('enables the pane-border header on first create and restores it when empty', async () => {
    ...
    expect(client.setWindowOption).toHaveBeenCalledWith('@1', 'pane-border-lines', 'double');
    expect(client.setWindowOption).toHaveBeenCalledWith('@1', 'pane-border-style', 'fg=colour240');
    expect(client.setWindowOption).toHaveBeenCalledWith('@1', 'pane-active-border-style', 'fg=colour45');
    ...
    expect(client.unsetWindowOption).toHaveBeenCalledWith('@1', 'pane-border-lines');
    expect(client.unsetWindowOption).toHaveBeenCalledWith('@1', 'pane-border-style');
    expect(client.unsetWindowOption).toHaveBeenCalledWith('@1', 'pane-active-border-style');
  });
  ...
  test('acquire stores the run description in @smux_description and the header label', async () => {
    ...
    const updates = (client.updatePane as ReturnType<typeof vi.fn>).mock.calls.at(-1)!;
    const options = updates[2] as { name: string; value: string | null }[];
    expect(options).toContainEqual({
      name: '@smux_description',
      value: 'lint gate before release',
    });
    const label = options.find((option) => option.name === '@smux_label')?.value;
  ...
  test('SIDEMUX_PANE_HEADER=0 leaves the window border untouched', async () => {
    const client = stubClient();
    const allocator = new PaneAllocator(client, loadConfig({ SIDEMUX_PANE_HEADER: '0' }), {}, '/proj');
    await allocator.acquire({ name: 'test', command: 'pnpm test' });
  ...
  describe('reuse (strict affinity)', () => {
    ...
    test('reuse disabled via config always creates', async () => {
      const client = stubClient();
      const allocator = new PaneAllocator(client, loadConfig({ SIDEMUX_REUSE_PANES: '0' }), {}, '/proj');
    ...
    test('a restarted server in the same cwd reclaims its panes', async () => {
      const client = stubClient();
      const before = new PaneAllocator(client, loadConfig({}, '/proj'), {}, '/proj');
      ...
      const after = new PaneAllocator(client, loadConfig({}, '/proj'), {}, '/proj');
    ...
    test('a stale busy pane from a dead server becomes reusable', async () => {
      const client = stubClient();
      const allocator = new PaneAllocator(client, loadConfig({}, '/proj'), {}, '/proj');
      ...
      const restarted = new PaneAllocator(client, loadConfig({}, '/proj'), {}, '/proj');
  ...
  describe('garbage collection', () => {
    test('startup keybind scheduling does not wait for garbage collection', async () => {
      ...
      await expect(allocator.ensureWorkspaceKeybinds()).resolves.toBeUndefined();
    ...
    test('skips windows with a genuinely busy pane (live server pid)', async () => {
      ...
      await client.setPaneOption(stale.paneId, '@smux_server_pid', String(process.pid));
      await client.setWindowOption('@1', '@smux_server_pid', '0');
  ...
  describe('workspace keybind', () => {
    test('default Prefix e opens the first-party dashboard', async () => {
      ...
      expect(binding?.slice(0, 4)).toEqual(['-T', 'prefix', 'e', 'display-popup']);
      expect(binding).toEqual(
        expect.arrayContaining(['-E', '-w', '96%', '-h', '92%', '-x', 'C', '-y', 'C']),
      );
    ...
    test('dashboard key is configurable', async () => {
      const client = stubClient();
      const allocator = new PaneAllocator(client, loadConfig({ SIDEMUX_DASHBOARD_KEY: 's' }), {}, '/proj');
      await allocator.acquire({ name: 'test' });

      const binding = vi.mocked(client.bindKey).mock.calls[0]?.[0];
      expect(binding?.slice(0, 4)).toEqual(['-T', 'prefix', 's', 'display-popup']);
    ...
    test('keybind can be installed on process startup before a run', async () => {
      ...
      expect(client.bindKey).toHaveBeenCalledTimes(1);
      const command = vi.mocked(client.bindKey).mock.calls[0]?.[0]?.at(-1) ?? '';
      expect(command).toContain("SIDEMUX_SESSION='smux'");
      expect(command).toContain('dashboard');
    });
    ...
    test('keybind retries after a failed bind', async () => {
      const client = stubClient({
        bindKey: vi.fn().mockRejectedValueOnce(new Error('tmux busy')).mockResolvedValue(undefined),
      });
      const allocator = new PaneAllocator(client, loadConfig({}), {}, '/proj');

      await expect(allocator.ensureWorkspaceKeybinds()).rejects.toThrow('tmux busy');
      await allocator.ensureWorkspaceKeybinds();
    ...
    test('rediscovering managed panes after restart refreshes the dashboard keybind', async () => {
      const client = stubClient();
      const beforeRestart = new PaneAllocator(client, loadConfig({}), {}, '/proj');
      const pane = await beforeRestart.acquire({ name: 'test', command: 'pnpm test' });
      vi.mocked(client.bindKey).mockClear();

      const afterRestart = new PaneAllocator(client, loadConfig({}), {}, '/proj');
