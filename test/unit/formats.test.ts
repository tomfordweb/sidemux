...
describe("parsePaneList", () => {
  ...
  test("decodes base64url-encoded names and commands", () => {
    ...
    expect(parsePaneList(out)[0]).toMatchObject({
      lastCommand: command,
      serverPid: null,
    });
  });
  ...
  test("throws on malformed line", () => {
    expect(() => parsePaneList("%0\tonly-two")).toThrow(
      /unexpected list-panes/,
    );
  });
});
...
describe("parseWindowList", () => {
  test("parses tmux window rows", () => {
    ...
    expect(
      parseWindowList(`smux\t0\t@1\tmain\t%1\tagent-1\t123\t456\t${stats}\n`),
    ).toEqual([
      {
        sessionName: "smux",
        windowIndex: "0",
  ...
  test("missing stats field reads as null", () => {
    expect(
      parseWindowList("smux\t0\t@1\tmain\t%1\tagent-1\t123\t456\n")[0]
        ?.statsJson,
    ).toBeNull();
  });
});
