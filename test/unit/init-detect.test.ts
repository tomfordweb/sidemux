...
describe('detectCandidates', () => {
  test('maps known scripts to roles under the detected manager, ordered by role', () => {
    const candidates = detectCandidates({
      packageJson: { scripts: { dev: 'vite', build: 'tsup', test: 'vitest', lint: 'eslint .' } },
  ...
  test('pyproject picks the runner from the lockfile (uv wins over poetry)', () => {
    ...
    expect(detectCandidates({ pyproject, rootFiles: ['uv.lock', 'poetry.lock'] })).toEqual([
      { role: 'test', command: 'uv run pytest', longRunning: false },
    ]);
  ...
  test('pyproject detects pytest from config section or pytest.ini', () => {
    expect(
      detectCandidates({ pyproject: { sections: ['tool.pytest.ini_options'], deps: [] } }),
    ).toEqual([{ role: 'test', command: 'pytest', longRunning: false }]);
    expect(
      detectCandidates({ pyproject: { sections: [], deps: [] }, rootFiles: ['pytest.ini'] }),
    ).toEqual([{ role: 'test', command: 'pytest', longRunning: false }]);
  });
  ...
  test('weak pytest signals (tests dir, tox.ini) need a runner lock', () => {
    ...
    expect(detectCandidates({ pyproject, rootFiles: ['tests', 'uv.lock'] })).toEqual([
      { role: 'test', command: 'uv run pytest', longRunning: false },
    ]);
  });
  ...
  test('pyproject detects ruff and mypy from sections or deps', () => {
    const candidates = detectCandidates({
      pyproject: { sections: ['tool.ruff.lint', 'tool.mypy.overrides'], deps: [] },
      rootFiles: ['uv.lock'],
    });
    expect(candidates).toEqual([
    ...
    expect(
      detectCandidates({ pyproject: { sections: [], deps: ['pytest-cov', 'ruff'] } }),
  ...
  test('conventional commands coexist with Makefile targets', () => {
    const candidates = detectCandidates({ rootFiles: ['go.mod'], makefileTargets: ['test'] });
  ...
  test('maps justfile recipes like Makefile targets', () => {
    const candidates = detectCandidates({ justfileRecipes: ['test', 'dev', 'fmt'] });
    expect(candidates).toEqual([
  ...
  test('polyglot projects merge candidates across manifests', () => {
    ...
    expect(candidates.map((c) => c.command)).toEqual(['uv run pytest', 'pnpm build']);
  });
});
...
describe('parsePyproject', () => {
  test('collects PEP 621 deps across multi-line arrays, extras intact', () => {
    ...
    expect(facts.sections).toEqual(['project', 'project.optional-dependencies', 'dependency-groups']);
  });
  ...
  test('normalizes section headers including array-of-tables', () => {
    const facts = parsePyproject(
      ['[tool.ruff.lint]', 'select = ["E"]', '', '[[tool.mypy.overrides]]', 'module = "x"'].join(
        '\n',
      ),
    );
  ...
  test('ignores mentions outside dependency contexts', () => {
    const facts = parsePyproject(
      ['[project]', 'name = "pytest-helper"', 'description = "works with pytest"'].join('\n'),
    );
    expect(facts.deps).toEqual([]);
  });
});
...
describe('matchesDelegated', () => {
  ...
  test('matches an exact or prefixed segment', () => {
    ...
    expect(matchesDelegated('pnpm test --coverage', delegated)).toBe('pnpm test');
  });

  test('matches inside a compound command', () => {
    expect(matchesDelegated('cd app && pnpm build', delegated)).toBe('pnpm build');
    expect(matchesDelegated('pnpm lint; pnpm test', delegated)).toBe('pnpm test');
  });
