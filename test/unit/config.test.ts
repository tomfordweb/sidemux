...
describe('loadConfig', () => {
  ...
  test('default agent id is stable per working directory', () => {
    ...
    expect(loadConfig({}, '/proj').agentId).toBe(loadConfig({}, '/proj').agentId);
    expect(loadConfig({}, '/proj').agentId).not.toBe(loadConfig({}, '/other').agentId);
  });
  ...
  test('uses CODEX_THREAD_ID when SIDEMUX_AGENT_ID is absent', () => {
    const config = loadConfig({ CODEX_THREAD_ID: '019f333c-acdd-72f0-8610-58ee1aab1346' });
  ...
  test('dashboard density accepts compact/normal/spacious and defaults invalid to normal', () => {
    ...
    expect(loadConfig({ SIDEMUX_DASHBOARD_DENSITY: 'compact' }).dashboardDensity).toBe('compact');
    expect(loadConfig({ SIDEMUX_DASHBOARD_DENSITY: 'SPACIOUS' }).dashboardDensity).toBe('spacious');
    const error = vi.spyOn(console, 'error').mockImplementation(() => {});
    expect(loadConfig({ SIDEMUX_DASHBOARD_DENSITY: 'dense' }).dashboardDensity).toBe('normal');
  ...
  test('invalid idle pane TTL falls back to default; 0 disables retention', () => {
    ...
    expect(loadConfig({ SIDEMUX_IDLE_PANE_TTL_MS: '-1' }).idlePaneTtlMs).toBe(DEFAULT_IDLE_PANE_TTL_MS);
    expect(loadConfig({ SIDEMUX_IDLE_PANE_TTL_MS: 'nope' }).idlePaneTtlMs).toBe(DEFAULT_IDLE_PANE_TTL_MS);
