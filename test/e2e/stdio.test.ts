...
describe.skipIf(!tmuxAvailable())('MCP stdio e2e against the built server', () => {
  ...
  test('exposes the expected tools, each with a teaching description', async () => {
    ...
    expect(names).toEqual(['close_all', 'kill', 'list_panes', 'read', 'run', 'send_keys', 'status', 'wait']);
  ...
  test('full loop: run → structured result → read → list_panes → kill-pane', async () => {
    const run = await client.callTool({
      name: 'run',
      arguments: { command: 'echo e2e-ok', description: 'e2e probe', name: 'e2e-job', timeout_ms: 15_000 },
    });
    ...
    const listOut = list.structuredContent as { panes: { pane: string; managed: boolean }[] };
  ...
  test('failing command round-trips its exit code', async () => {
    const run = await client.callTool({
      name: 'run',
      arguments: { command: 'sh -c "exit 42"', description: 'e2e failure probe', name: 'e2e-fail', timeout_ms: 15_000 },
    });
    expect(run.isError, JSON.stringify(run)).not.toBe(true);
