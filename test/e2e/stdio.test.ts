...
describe.skipIf(!tmuxAvailable())('MCP stdio e2e against the built server', () => {
  let client: Client;

  beforeAll(async () => {
    if (!existsSync(ENTRY)) {
      execFileSync('node', ['node_modules/tsup/dist/cli-default.js'], { cwd: ROOT, stdio: 'ignore' });
    }
  ...
  test('exposes exactly the seven tools, each with a teaching description', async () => {
    const { tools } = await client.listTools();
    const names = tools.map((t) => t.name).sort();
    expect(names).toEqual(['close_all', 'kill', 'list_panes', 'read', 'run', 'send_keys', 'wait']);
  ...
  test('full loop: run → structured result → read → list_panes → kill-pane', async () => {
    const run = await client.callTool({
      name: 'run',
      arguments: { command: 'echo e2e-ok', name: 'e2e-job', timeout_ms: 15_000 },
    });
    ...
    const readOut = read.structuredContent as { text: string; job_status: string };
    ...
    const listOut = list.structuredContent as { panes: Array<{ pane: string; managed: boolean }> };
    expect(listOut.panes.some((p) => p.pane === runOut.pane && p.managed)).toBe(true);
  ...
  test('failing command round-trips its exit code', async () => {
    const run = await client.callTool({
      name: 'run',
      arguments: { command: 'sh -c "exit 42"', name: 'e2e-fail', timeout_ms: 15_000 },
    });
    const out = run.structuredContent as { status: string; exit_code: number };
